...
import { createFileContentUrl } from "../utils/apiConfig";
  ...
  useEffect(() => {
    const fetchTestDetails = async () => {
      ...
      try {
        ...
        const { directory: testDirectory, flowName: testFlowName } =
          extractTestPathInfo(testId);
        setDirectory(testDirectory);
        setFlowName(testFlowName);
        setTestName(testFlowName); // Use flow name as test name

        const fileUrl = await createFileContentUrl(testId);
        ...
        if (Array.isArray(data)) {
          ...
          if (
            ...
            processedCommands[0].metadata?.timestamp
          ) {
