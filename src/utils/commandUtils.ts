...
export const getCommandName = (command: Command): string => {
  ...
  if (command.tapOnElementCommand) return command.tapOnElementCommand.longPress ? 'Long Press' : 'Tap';
  ...
  if (command.tapOnElement) return command.tapOnElement.longPress ? 'Long Press' : 'Tap';
...
export const isUnknownCommand = (command: Command): boolean => {
  ...
  return !command.assertConditionCommand && 
...
export const getCommandDetails = (command: Command): string => {
  ...
  if (command.assertConditionCommand) {
    ...
    if (condition?.visible) return `"${condition.visible.textRegex}" is visible`;
    if (condition?.notVisible) return `"${condition.notVisible.textRegex}" is not visible`;
  ...
  if (command.tapOnElementCommand) {
    ...
    return `${longPress ? 'Long press' : 'Tap'} on "${selector?.textRegex || 'Unknown'}"`;
  }
  ...
  if (command.runFlowCommand) {
    ...
    if (sourceDescription) {
      details = details ? `${details} (${sourceDescription})` : `Source: ${sourceDescription}`;
    }
    
    // Add condition information if there's no flow path
    if (!flow && condition) {
      if ('scriptCondition' in condition) {
        details = details ? `${details} when: ${condition.scriptCondition}` : `when: ${condition.scriptCondition}`;
      } else if (condition.visible) {
        details = details ? `${details} when: Visible "${condition.visible.textRegex}"` : `when: Visible "${condition.visible.textRegex}"`;
      } else if (condition.notVisible) {
        details = details ? `${details} when: Not Visible "${condition.notVisible.textRegex}"` : `when: Not Visible "${condition.notVisible.textRegex}"`;
      }
    }
  ...
  if (command.scrollUntilVisibleCommand) {
    ...
    return `Scroll ${direction} until "${element?.textRegex || 'Unknown'}" is visible`;
  }
  
  // New command types
  if (command.defineVariablesCommand) {
    const variables = Object.keys(command.defineVariablesCommand.env || {}).join(', ');
  ...
  if (command.scrollUntilVisible) {
    ...
    return `Scroll ${direction} until "${selector?.textRegex || 'Unknown'}" is visible`;
  }
  
  if (command.setAirplaneModeCommand) {
    return `Airplane Mode: ${command.setAirplaneModeCommand.value ? 'Enabled' : 'Disabled'}`;
  }
...
export const formatTimestamp = (timestamp: number | undefined | null, index: number, startTime: number | null): string => {
