...
app.get('/api/test/:testId', async (req: Request, res: Response) => {
  try {
    ...
    const metadata = await fs.readFile(path.join(testPath, 'metadata.json'), 'utf-8')
      .catch(() => '{}');
...
app.get('/api/test/:testId/steps', async (req: Request, res: Response) => {
  try {
    ...
    const testPath = path.join(debugDir, testId);
    const stepsFile = await fs.readFile(path.join(testPath, 'steps.json'), 'utf-8')
      .catch(() => '[]');
...
app.get('/api/screenshot', async (req: Request, res: Response) => {
  try {
    ...
    const matchingScreenshot = files.find(file => 
...
app.get('/api/command-images/:commandName', async (req: Request, res: Response) => {
  try {
    const debugDir = req.query.dir as string;
    ...
    const matchingImages = files.filter(file => {
      return file.includes(commandName) && 
    ...
    res.json({
      images: sortedImages.map(image => ({
        ...
        url: `/api/files/content?path=${encodeURIComponent(path.join(debugDir, image))}`
      }))
    });
  } catch (error) {
...
app.get('/api/flow-images', async (req: Request, res: Response) => {
  try {
    ...
    if (!directory || !flowName) {
      return res.status(400).json({ error: 'Directory and flow name are required' });
    }

    // Read all files in the directory
    const files = await fs.readdir(directory as string);
    
    // Filter for image files that match the flow name
    const matchingImages = files.filter(file => {
      return file.includes(flowName as string) && 
             (file.endsWith('.png') || file.endsWith('.jpg') || file.endsWith('.jpeg'));
    });

    // Extract timestamp and sort images
    const imagesWithTimestamps = matchingImages.map(image => {
      // Extract timestamp from filename (assuming format includes timestamp)
      const timestampMatch = image.match(/\d{13}/); // Matches 13-digit timestamp
      const timestamp = timestampMatch ? parseInt(timestampMatch[0]) : 0;
      
      return {
        filename: image,
        timestamp,
        path: path.join(directory as string, image),
        url: `/api/files/content?path=${encodeURIComponent(path.join(directory as string, image))}`
      };
    });

    // Sort by timestamp
    const sortedImages = imagesWithTimestamps.sort((a, b) => a.timestamp - b.timestamp);
